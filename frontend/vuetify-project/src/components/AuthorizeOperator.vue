        ...
        <v-row justify="center">
            <v-col cols=auto>
                <v-card width="700" height="210" variant="outlined">
                    <v-card-title>Authorize Operator:</v-card-title>
                    <v-card-text>
                        <v-text-field label="Enter Operator Address" prepend-icon="mdi-pencil" v-model="operatorAddress"></v-text-field>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="primary" :disabled="!operatorAddress" @click="authorizeOperator">Authorize Operator</v-btn>
                    </v-card-actions>
                </v-card>
                <br>
...
export default {
    ...
    methods: {
        async authorizeOperator() {
            ...
            try {
                console.log(this.operatorAddress);
                const response = await axios.post('http://localhost:3003/authorizeOperator', {operatorAddress: this.operatorAddress});
